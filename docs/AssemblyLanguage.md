...
![Alt text](G:\dev\docsify\docs\images\assembly\image-20231129084354423.png)

#### 内存地址空间：

*  最终运行程序的是CPU,开发者使用汇编编程的时候，必须要从CPU角度考虑问题。
...
一个CPU由运算器、控制器、寄存器等器件组成，这些器件靠内部总线相连。
...
![Alt text](G:\dev\docsify\docs\images\assembly\image-20231129141359846.png)
...
![image-20231129141606309](C:\Users\huawei\AppData\Roaming\Typora\typora-user-images\image-20231129141606309.png)

一个16位寄存器可以存储16位的数据，一个16位寄存器所能存储的数据如下所示
$$
2^{16}-1
$$
...
![Alt text](G:\dev\docsify\docs\images\assembly\image-20231129143340411.png)
...
二进制数0100111000100000本身也可表示一个数值的大小，但人类习惯10进制，用十进制20000表示可以让我们直观的感受到这个数值大小，十六进制数的一位相当于二进制数的四位，如0100111000100000可表示4(0100)、E（1110）、2（0010）、0（0000）四位十六进制数。

由于一个内存单元可存放8位数据，CPU中的寄存器又可存放n个8位的数据。也就是说，计算机中的数据大多是由1~N个8位数据构成的，很多时候，需要直观地看出组成数据的各个字节数据的值，用16进制来表示数据可以直观地看出这个数据是由哪些8位数据构成的。比如20000写成4E20就可以直观地看出，这个数据是由4E和20两个8位数据构成的，如果AX中存放4E80，则AH里是4E，AL里是20。这种表示方法便于许多问题的直观分析，我们多用16进制来表示一个数据。为了区分不同的进制，在16进制表示的数据后面加上H，在二进制表示的数据后面加上B，十进制的数据后面什么也不加。

如：可用3中不同的进制标示图表示AX里的数据
...
| 指令      | 控制CPU完成的操作                  | 用高级语言的语法描述 |
| --------- | ---------------------------------- | -------------------- |
| mov ax,18 | 将18送入AX                         | AX=18                |
| mov ah,78 | 将78送入AH                         | AH=78                |
| add ax,8  | 将寄存器AX中的数值加上8            | AX=AX+8              |
| mov ax,bx | 将寄存器BX中的数据宋瑞寄存器AX     | AX=BX                |
| add ax,bx | 将AX，BX中的内容相加，结果存在AX中 | AX=AX+BX             |



CPU執行的程序段的每条指令后，对寄存器中的数据进行的改变

原ax的值是0000H

原bx的值是0000H

| 程序段中的指令 | 程序执行后后AX中的数据 | 指令执行后BX中的数据 |
| -------------- | ---------------------- | -------------------- |
...
add ax, bx bx 的值已经是8226H了，但是为什么是044CH呢，其实8226+8226 根据16进制位计算是1044C, CPU在写入AX数据的时候，它会把1去掉，把044C加上H写入进AX中。1作为进制位放到其他位置去。

### 程序段中指令执行情况2（原AX中的值：0000H，原BX中的值0000H）

| 程序段中的指令 | 指令执行后AX中的数据 | 指令执行后BX中的数据 |
| -------------- | -------------------- | -------------------- |
| mov ax,0001AH  | 001AH                | 0000H                |
...
| add al,93H     | 0022H                | 4026H                |

这里的丢失，指的是进位制不等你在8位寄存器中保存，但是CPU不是真正的丢弃这个进位值。CPU由专门的位置存放进位值

写吹每条汇编指令执行后相关寄存器的值

| 程序段中的指令 | 指令执行后AX\BX\CX中的数据 |
| -------------- | -------------------------- |
| mov ax,62627   | AX=                        |
| mov ah,31H     | AX=                        |
| mov al,23H     | AX=                        |
